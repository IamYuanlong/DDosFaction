...
//		float sever = Math.abs((s1.get(4) - s1.get(2)) / 2- (s1.get(2) - s1.get(0)) / 2);
		float sever = 8-(float)1.0*((s1.get(4) +s1.get(3)+s1.get(2) + s1.get(0)+s1.get(1))/5);
		return sever;
	...
	public static float getNetSever(int accpsum, int sendsum) {
		...
		if (accpsum > 0) {
			float sumsever = sendsum / (float)1.0*accpsum;
			return sumsever;
	...
	public static Map<String, Integer> getUnnomalMap() {
		...
		mapforrules = MyPcapPacketHandler.getMapfordosip();
