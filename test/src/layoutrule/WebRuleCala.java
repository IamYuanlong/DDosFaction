...
public class WebRuleCala {
	...
	public static float getCPUGradientAve(List<Float> sFloats) {
		...
		s1 = sFloats;
		// cpu占比的计算
		float sever = Math.abs((s1.get(AllInfo.getCpusever().size()-1) +s1.get(AllInfo.getCpusever().size()-3)+s1.get(AllInfo.getCpusever().size()-2)) / 3 - (s1.get(AllInfo.getCpusever().size()-3) + s1.get(AllInfo.getCpusever().size()-4)+s1.get(AllInfo.getCpusever().size()-5)) / 3);
		// cpu占比的服务率
		return sever;
	}
	...
	public static float getHttpServer(int httprequste, int httpreponse) {
		...
		if (httprequste > 0) {
			s =(float)1.0*httpreponse /(float)1.0* httprequste;
		} else {
			s = 0;
		}
		return s;
